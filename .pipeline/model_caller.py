...
def get_model_info(role: str) -> dict:
    global _caller
    if _caller is None:
        _caller = ModelCaller()
    try:
        role_name = _normalize_role_name(role)
        cfg = _caller.roles[role_name]
        return {"role": role_name, "provider": cfg.get("provider_type", "unknown"), "model": cfg.get("model", "unknown")}
    except Exception:
        return {"role": role, "provider": "unknown", "model": "unknown"}
